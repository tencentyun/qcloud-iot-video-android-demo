...
| IV_ERR_SYS_FILE_OPT_FAIL       | -107 | 文件操作错误                                 |
| IV_ERR_SYS_MALLOC_FAIL         | -108 | 申请内存错误                                 |
| IV_ERR_SYS_COMMUNICATE_FAIL    | -109 | 与服务器通信出错                             |
| IV_ERR_SYS_COMMUNICATE_TIMEOUT | -110 | 与服务器通信超时                             |
| IV_ERR_SYS_MAX_RETRY_COUNT     | -111 | 已达到最大重试次数                           |
...
| IV_ERR_AVT_STATE_ERROR          | -312 | 状态错误                               |
| IV_ERR_AVT_VOIP_NOT_ACTIVATED   | -350 | 微信视频通话功能未激活                 |
| IV_ERR_AVT_VOIP_EXPIRED         | -351 | 微信视频通话功能已过期                 |
...
| IV_ERR_CS_NO_BALANCE                | -612 | 无云存套餐                 |
| IV_ERR_CS_HTTP_DATA_FAIL            | -613 | 云存http上传失败           |
