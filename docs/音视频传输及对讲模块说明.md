...
- [3. 接口参考](#3-接口参考)
  - [接口列表](#接口列表)
  - [接口描述](#接口描述)
    - [iv_avt_init](#iv_avt_init)
    - [iv_avt_exit](#iv_avt_exit)
    - [iv_avt_send_stream](#iv_avt_send_stream)
    - [iv_avt_get_send_stream_status](#iv_avt_get_send_stream_status)
    - [iv_avt_get_send_stream_buf](#iv_avt_get_send_stream_buf)
    - [iv_avt_send_finish_stream](#iv_avt_send_finish_stream)
    - [iv_avt_recv_finish_stream](#iv_avt_recv_finish_stream)
    - [iv_avt_send_command](#iv_avt_send_command)
    - [iv_avt_get_peerinfo](#iv_avt_get_peerinfo)
    - [iv_avt_set_mtu](#iv_avt_set_mtu)
    - [iv_avt_get_mtu](#iv_avt_get_mtu)
    - [iv_avt_get_av_enc_info_cb](#iv_avt_get_av_enc_info_cb)
    - [iv_avt_start_real_play_cb](#iv_avt_start_real_play_cb)
    - [iv_avt_stop_real_play_cb](#iv_avt_stop_real_play_cb)
    - [iv_avt_start_recv_stream_cb](#iv_avt_start_recv_stream_cb)
    - [iv_avt_stop_recv_stream_cb](#iv_avt_stop_recv_stream_cb)
    - [iv_avt_recv_stream_cb](#iv_avt_recv_stream_cb)
    - [iv_avt_recv_user_data_cb](#iv_avt_recv_user_data_cb)
    - [iv_avt_notify_cb](#iv_avt_notify_cb)
    - [iv_avt_recv_command_cb](#iv_avt_recv_command_cb)
    - [iv_avt_download_file_cb](#iv_avt_download_file_cb)
    - [iv_avt_get_peer_outer_net_cb](#iv_avt_get_peer_outer_net_cb)
    - [iv_avt_ai_start](#iv_avt_ai_start)
    - [iv_avt_ai_stop](#iv_avt_ai_stop)
- [4. 数据结构](#4-数据结构)
  - [数据结构列表](#数据结构列表)
  - [数据结构描述](#数据结构描述)
    - [iv_avt_stream_type_e](#iv_avt_stream_type_e)
    - [iv_avt_device_status_e](#iv_avt_device_status_e)
    - [iv_avt_request_type_e](#iv_avt_request_type_e)
    - [iv_avt_video_res_type_e](#iv_avt_video_res_type_e)
    - [iv_avt_usr_data_parm_s](#iv_avt_usr_data_parm_s)
    - [iv_avt_req_stream_info_s](#iv_avt_req_stream_info_s)
    - [iv_avt_req_stream_param_s](#iv_avt_req_stream_param_s)
    - [iv_avt_chn_name_param_s](#iv_avt_chn_name_param_s)
    - [iv_avt_event_e](#iv_avt_event_e)
    - [iv_avt_command_type_e](#iv_avt_command_type_e)
    - [iv_avt_download_status_e](#iv_avt_download_status_e)
    - [iv_avt_p2p_log_level_e](#iv_avt_p2p_log_level_e)
    - [iv_avt_p2p_nic_type_e](#iv_avt_p2p_nic_type_e)
    - [iv_avt_p2p_tp_type_e](#iv_avt_p2p_tp_type_e)
    - [congestion_ctrl_s](#congestion_ctrl_s)
    - [p2p_keep_alive_s](#p2p_keep_alive_s)
    - [iv_p2p_send_stats_s](#iv_p2p_send_stats_s)
    - [p2p_init_params](#p2p_init_params)
    - [local_net_info_s](#local_net_info_s)
    - [iv_avt_init_parm_s](#iv_avt_init_parm_s)
    - [iv_ai_start_s](#iv_ai_start_s)
- [5. 注意事项](#5-注意事项)
  - [内存消耗统计方法](#内存消耗统计方法)
- [6. 示例代码](#6-示例代码)
...
**接收对端音视频流程**  
```
iv_avt_start_recv_stream_cb //开始接收音视频回调
    |
...
- iv_avt_recv_finish_stream 设备端停止接收对端发送的音视频流
...
- iv_avt_get_peer_outer_net_cb 获取对端外网IP信息回调
...
**使用说明**  
调用该接口设备端能够停止接收流，可用于双向音视频时主动停止接收对端发送过来的音视频流
...
受小程序功能限制，msg要求为json格式，长度由msg_len提供，建议在16KB以内，否则会影响实时性  
json消息内请勿包含"iv_private_cmd"字段，该字段系统内部保留使用  
...
| 返回值   | 描述               |
| -------- | ------------------ |
| peerinfo | peerinfo的实际信息 |

**说明**  
该接口为了及时获取最准确的peerinfo，推荐`IV_AVT_EVENT_P2P_PEER_READY`事件回调中调用
...
| 参数名称 | 类型     | 描述    | 输入/输出 |
| -------- | -------- | ------- | --------- |
| mtu_size | uint32_t | MTU数值 | 输入      |

**返回值**  
无

**说明**  
该接口为主动配置MTU大小，范围 500-1200，
当播放端不出图时，建议调用该接口，逐渐递减MTU值。
...
**说明**  
已废弃，请勿使用，出于兼容目的暂时保留此接口，后续版本可能删除
...
**功能描述**
开启twetalk AI对话。必须要等`av_talk_init();`初始化成功后，才可以调用。
...
- iv_avt_req_stream_info_s 请求流信令信息
...
| IV_AVT_VIDEO_AUDIO    | 音频流 | 4    |
| IV_AVT_VIDEO_RES_BUTT | 无效流 | -    |
...
**功能描述**  
`iv_avt_start_real_play_cb`回调对应的请流信息
...
| --------- | --------------------------------------------------------- | ---- |
| user_args | 用户自定义参数                                            | -    |
| requester | 请流角色，参考对应信令说明                                | -    |
| pb_time   | 回放时间，流类型为IV_AVT_VIDEO_RES_PB参数有效，其他为NULL | -    |

### iv_avt_req_stream_param_s

**功能描述**  
`IV_AVT_COMMAND_REQ_STREAM` 事件对应的参数  
手机可以在发起视频拉流请求前使用`get_device_st`信令查询设备状态，设备可根据自身情况接受或拒绝本次拉流请求
...
typedef enum
{
    ...
    IV_AVT_EVENT_P2P_LINK_DISCONNECT   = 8,
    IV_AVT_EVENT_P2P_CLI_SEND_CLOSE    = 9,
...
| IV_AVT_EVENT_P2P_LINK_DISCONNECT   | P2P client端断开连接       | 8    |
| IV_AVT_EVENT_P2P_CLI_SEND_CLOSE    | P2P client端结束发送数据流 | 9    |
| IV_AVT_EVENT_P2P_BUTT              | 无效事件                   | -    |

**注意事项**  
出现`IV_AVT_EVENT_P2P_PEER_ADDR_CHANGED`事件，SDK 内部会进行重新协商，但此时P2P通道已断开，需要停止送入数据
出现`IV_AVT_EVENT_P2P_PEER_ERROR` 或 `IV_AVT_EVENT_P2P_PEER_CONNECT_FAIL` 事件时，如果用户使用局域网模式，就不用处理这两个事件；如果用户使用广域网通信，偶尔收到这两个事件属于正常现象，如果短期内频繁收到说明用户所在网络环境复杂导致难以维持正常连接，建议更换网络。
...
typedef enum
{
    ...
    IV_AVT_COMMAND_PLAYBACK_PROGRESS,
    IV_AVT_COMMAND_QUERY_FILE_LIST,
    IV_AVT_COMMAND_CALL_ANSWER,
    IV_AVT_COMMAND_CALL_HANG_UP,
    IV_AVT_COMMAND_CALL_REJECT,
    IV_AVT_COMMAND_CALL_CANCEL,
    IV_AVT_COMMAND_CALL_BUSY,
    IV_AVT_COMMAND_CALL_TIMEOUT,
    IV_AVT_COMMAND_CALL_AV_CLOSE,
    IV_AVT_COMMAND_CALL_AV_AUDIO_ONLY,
    IV_AVT_COMMAND_CALL_AV_VIDEO_ONLY,
    IV_AVT_COMMAND_CALL_AV_AUDIO_VIDEO,
...
| IV_AVT_COMMAND_PLAYBACK_PROGRESS    | 查询播放进度                                              | 12   |
| IV_AVT_COMMAND_QUERY_FILE_LIST      | 查询文件列表                                              | 13   |
| IV_AVT_COMMAND_CALL_ANSWER          | 用于twecall呼叫场景，表示对方接听                         | 14   |
| IV_AVT_COMMAND_CALL_HANG_UP         | 用于twecall呼叫场景，表示对方挂断                         | 15   |
| IV_AVT_COMMAND_CALL_REJECT          | 用于twecall呼叫场景，表示对方拒接（预留，未使用）         | 16   |
| IV_AVT_COMMAND_CALL_CANCEL          | 用于twecall呼叫场景，表示本方主动取消呼叫（预留，未使用） | 17   |
| IV_AVT_COMMAND_CALL_BUSY            | 用于twecall呼叫场景，表示对方占线（预留，未使用）         | 18   |
| IV_AVT_COMMAND_CALL_TIMEOUT         | 用于twecall呼叫场景，表示对方无人接听（预留，未使用）     | 19   |
| IV_AVT_COMMAND_CALL_AV_CLOSE        | 用于twecall呼叫场景，表示对方麦克风，摄像头都关闭         | 20   |
| IV_AVT_COMMAND_CALL_AV_AUDIO_ONLY   | 用于twecall呼叫场景，表示对方只开启麦克风                 | 21   |
| IV_AVT_COMMAND_CALL_AV_VIDEO_ONLY   | 用于twecall呼叫场景，表示对方只开启摄像头                 | 22   |
| IV_AVT_COMMAND_CALL_AV_AUDIO_VIDEO  | 用于twecall呼叫场景，表示对方开启麦克风和摄像头           | 23   |
...
| IV_AVT_P2P_AUTO | P2P自动探测协议 | 2    |

**使用说明**
请优先使用UDP协议建立连接，使用TCP协议建立连接会导致额外的资源开销。  
...
- 当`IoT Video SDK`内部缓存的数据超过`high_mark`时，会触发一次`IV_AVT_EVENT_P2P_WATERMARK_HIGH`事件通知，表明内存缓存太多，网络环境太差，用户需要降低或停止数据流的发送，此时如果继续发送则会因网络发送速率过慢导致缓存持续增长，带来更多的内存开销
...
| proxy_ip             | 低功耗场景在WiFi模块运行proxy服务的IP地址字符串                              | 有效的IPv4地址字符串，为空则不进入proxy模式 |
| proxy_port           | 低功耗场景在WiFi模块运行proxy服务的端口号                                    | 有效的端口号，为0则不进入proxy模式          |
| loopback_port        | SDK部分功能需要使用本地回环上的特定端口，此接口可指定端口号，以免冲突        | 有效的端口号，0则使用默认端口号34567        |
| thread_policy        | 线程调度策略（保留参数，暂未使用）                                           | 0                                           |
| thread_priority      | 线程优先级（保留参数，暂未使用）                                             | 0                                           |
...
6. Twetalk AI对话
```c
int iot_video_ai_start(void)
{
    // 测试开启AI 这里参数需要转成json字符串的方式传入
    // 参数的具体含义和格式，请参考  https://cloud.tencent.com/document/product/647/115413
#define LLMCONFIG_TEST_STRING "{\\\"LLMType\\\": \\\"openai\\\",\\\"Model\\\":\\\"ChatGPT\\\",\\\"APIKey\\\":\\\"12345678\\\",\\\"APIUrl\\\": \\\"https://api.xxx.com/chat/completions\\\", \\\"Streaming\\\": true}"
#define TTSCONFIG_TEST_STRING "{\\\"AppId\\\": 20241212, \\\"TTSType\\\": \\\"TTSType\\\",\\\"SecretId\\\": \\\"your_secret\\\",\\\"SecretKey\\\":  \\\"your_key\\\",\\\"VoiceType\\\": 101001,\\\"Speed\\\": 1.25,\\\"Volume\\\": 5,\\\"PrimaryLanguage\\\": \\\"zh-CN\\\"}"
#define STTCONFIG_TEST_STRING "{\\\"Language\\\": \\\"zh\\\",\\\"VadSilenceTime\\\":\\\"500\\\"}"
#define AGENTCONFIG_TEST_STRING "{\\\"WelcomeMessage\\\":\\\"bot test\\\", \\\"InterruptSpeechDuration\\\":300}"

    iv_ai_start_s params;
    params.LLMConfig = LLMCONFIG_TEST_STRING;
    params.LLMLen    = strlen(LLMCONFIG_TEST_STRING);
    params.TTSConfig = TTSCONFIG_TEST_STRING;
    params.TTSLen = strlen(TTSCONFIG_TEST_STRING);
    params.STTConfig = STTCONFIG_TEST_STRING;
    params.STTLen = strlen(STTCONFIG_TEST_STRING);
    params.AgentConfig = AGENTCONFIG_TEST_STRING;
    params.AgentConfigLen = strlen(AGENTCONFIG_TEST_STRING);
    printf("iv avt ai start ...\r\n");
    return iv_avt_ai_start(&params);
}

int iot_video_ai_stop(void)
{
    return iv_avt_ai_stop();
}
```