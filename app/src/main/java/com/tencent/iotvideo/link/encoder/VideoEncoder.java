...
public class VideoEncoder {
    ...
    public VideoEncoder(VideoEncodeParam param, Activity activity) {
        if (activity != null) {
    ...
    public void setVideoBitRate(int bitRate) {
        ...
        if ((bitRate > beginBitRate) || (bitRate < MIN_BITRATE_LENGTH) || (nowBitrate == bitRate) || (bitRate > MAX_BITRATE_LENGTH)) {
            return;
        }
...
//                        Log.e("TAG", "==========P帧===============" + seq);
     ...
     * @return 旋转后的数据
     */
    private byte[] rotateNV270(byte[] data, int imageWidth, int imageHeight) {
        byte[] yuv = new byte[imageWidth * imageHeight * 3 / 2];
        // Rotate the Y luma
        int i = 0;
